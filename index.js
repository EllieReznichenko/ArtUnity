import{a as v,S as D,N as $,P as O,R as F}from"./assets/vendor-BuJqGh_H.js";(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const a of n)if(a.type==="childList")for(const i of a.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&s(i)}).observe(document,{childList:!0,subtree:!0});function t(n){const a={};return n.integrity&&(a.integrity=n.integrity),n.referrerPolicy&&(a.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?a.credentials="include":n.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function s(n){if(n.ep)return;n.ep=!0;const a=t(n);fetch(n.href,a)}})();const Y="https://sound-wave.b.goit.study/api",L=8;function R(e){alert(e)}async function T(e=1,r=8){const t=await fetch(`${Y}/artists?page=${e}&limit=${r}`);if(!t.ok)throw new Error("Не вдалося завантажити артистів");return await t.json()}const U="https://sound-wave.b.goit.study/api/feedbacks";async function N(){try{const e=await fetch(U);if(!e.ok)throw new Error("Failed to load feedbacks");return(await e.json()).data}catch(e){return console.error(e),[]}}const m={artistCardsContainer:document.querySelector("#artists-grid"),loadMoreBtn:document.getElementById("load-more-btn")};function j({_id:e,strArtist:r,genres:t,strArtistThumb:s,strBiographyEN:n}){const a=(t||[]).map(d=>`<span class="genre">${d}</span>`).join(""),i=(n==null?void 0:n.length)>100?n.slice(0,100)+"...":n||"",o=s&&typeof s=="string"&&s.trim()!==""?s:"/img/artists/Placeholder_Image.jpg";return`
  ...
  `}let f=1;async function M(){try{const e=await T(f,L);if(!e.artists||!Array.isArray(e.artists))throw new Error("Немає артистів у відповіді");const r=e.artists.map(j).join("");m.artistCardsContainer.insertAdjacentHTML("beforeend",r),f*L>=e.total?m.loadMoreBtn.style.display="none":m.loadMoreBtn.style.display="block",f++}catch(e){console.error(e),R("Помилка завантаження артистів"),m.loadMoreBtn.style.display="none"}}m.loadMoreBtn.addEventListener("click",M);document.addEventListener("DOMContentLoaded",()=>{M()});const A=document.querySelector(".modal"),c=A.querySelector(".modal-content"),y=document.querySelector(".loader");y||console.warn("Loader element not found in DOM!");let b=[];function H(){A.classList.add("is-open"),document.body.style.overflow="hidden"}function h(){A.classList.remove("is-open"),document.body.style.overflow="",c.innerHTML="",x()}function x(){b.forEach(({element:e,handler:r})=>{e.removeEventListener("click",r)}),b=[]}function V(){c.querySelectorAll(".youtube-btn").forEach(r=>{const t=()=>{const s=r.dataset.youtube;s&&window.open(s,"_blank")};r.addEventListener("click",t),b.push({element:r,handler:t})})}function q(e){var w;const r=e.yearFormed?e.yearDisbanded?`${e.yearFormed} - ${e.yearDisbanded}`:`${e.yearFormed} - present`:"information missing",t=e.gender||"information missing",s=e.membersCount||"information missing",n=e.country||"information missing",a=e.biography||"information missing";let i=[];Array.isArray(e.genres)?i=e.genres.filter(Boolean):typeof e.genres=="string"&&(i=e.genres.split(",").map(l=>l.trim()).filter(Boolean));const o=i.length?i.map(l=>`<span class="genre">${l}</span>`).join(" "):"information missing",d=e.imageUrl||"",u=(w=e.albums)!=null&&w.length?`<div class="albums-grid">${e.albums.map(l=>{const S=`
            <tr>
              <th>Track</th>
              <th>Time</th>
              <th>Link</th>
            </tr>`,B=l.tracks.map(g=>{const I=g.youtubeUrl?`<a href="${g.youtubeUrl}" target="_blank" rel="noopener noreferrer">YouTube</a>`:"";return`
                <tr>
                  <td>${g.title}</td>
                  <td>${g.duration}</td>
                  <td>${I}</td>
